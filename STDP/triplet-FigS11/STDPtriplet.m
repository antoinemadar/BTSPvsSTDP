function output = STDPtriplet(params)
%
...
Pdecay2 = params.Pdecay2; % triplet variable convoluted with presynaptic spikes
...
for i = 1:length(Trun)-1
    ...
    D2(i+1) = D2(i) - dt*D2(i)/Ddecay2 + OutRaster(i); % postsynaptic triplet variable
    for n = 1:N % for each presynaptic neuron
        ...
        P2(n,i+1) = P2(n,i) - dt*P2(n,i)/Pdecay2 + InRasters(n,i);
        Pact(n,i+1) = OutRaster(i)*P(n,i+1)*(Pamp + Damp2*D2(i));
        Dact(n,i+1) = InRasters(n,i)*D(i+1)*(Damp + Pamp2*P2(n,i));
        ...
        elseif WUdyn == 1 % dynamic update with time constant tau_wu
            Wtarget(n,i+1) = Wtarget(n,i) + Pact(n,i) - Dact(n,i);
