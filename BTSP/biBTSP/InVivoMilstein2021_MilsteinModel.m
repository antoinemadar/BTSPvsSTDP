...
speed = 15; %15; % cm/sec, (15cm/s average in DongSheffield2021, 25cm/s in Milstein2021 network model)
...
Params.Wmax = 125e-12; % Wmax, at most, should be 1.5*85pA=127.5pA, which results in ~37mV peak FRout, i.e, the upper limit for realistic PFs. 
...
addCS = 1; % 0 if no predefined CS, 1 if you want to simulate artificial CS induction
CSlap = 11%:13; % lap on which CS will occur if addCS=1
CStime = -9; %-L./(2.*speed); % time before COM of initial PF, in sec. If defined as empty array, it puts CS at a time point corresponding to input 46 COM (closest to 77pA)
...
NewLapIdx = find(Run==0); % segment in laps
NewLapTimes = Trun(NewLapIdx);
EndLapIdx = find(Run==L);
laps_end = zeros(size(Run));
laps_end(EndLapIdx) = 1;
% lapBounds.start = zeros(size(Run));
% lapBounds.start(NewLapIdx) = 1;
...
% CS trace: 
CS = zeros(1,length(Trun)); % CS trace -> 1 CS = 1 sample point
plateau = CS;
if addCS == 1
%add 1 CS 3s before initial COM of current PF (which is in the middle)
    ...
    CSidx = COM1idx_Laps(CSlap)-(CStime./dt);
    CS(CSidx) = 1;
    for l = 1:length(CSlap)
    plateau(CSidx(l):CSidx(l)+300) = 1; % 300ms plateau
    end
end

% run LIF + plasticity model
Params.dt = dt; Params.Imax = Imax;
[I, V, V2, plateau, OutRaster, D, P, SRA, W, W2]  = MilsteinBTSP_LIFadapt_ODEs(Params, W, InRasters, plateau, laps_end);
...
for lap = 1:Nlaps
    ...
    CSbin(lap,:) = histcounts(CSloc{lap}, TrackBins); % number of CSs in each spatial bins, for given lap
    CSbin2(lap,:) = histcounts(CSloc{lap}, TrackBins2);
...
InputNum = round(N/2); %dWmaxIn; %Nbittner
maxD = max(D(NewLapIdx(LapNum):NewLapIdx(LapNum+1)-1));
maxP = max(P(InputNum, NewLapIdx(LapNum):NewLapIdx(LapNum+1)-1));
normD = D(NewLapIdx(LapNum):NewLapIdx(LapNum+1)-1)./maxD;
normP = P(InputNum, NewLapIdx(LapNum):NewLapIdx(LapNum+1)-1)./maxP;
subplot(5,1,1)
    ...
    box off;
subplot(5,1,2)
    plot(Tlap1, normD.*normP, 'r'); hold on
    xlim([x1 x2]);
    xlabel('time (s)'); 
    title('normIS*normET')
    box off;
subplot(5,1,3)
    plot(Tlap1, q_sigmo(normD.*normP, Params.a_pot,Params.b_pot), 'r'); hold on
    plot(Tlap1, q_sigmo(normD.*normP, Params.a_dep,Params.b_dep), 'b'); hold on
    xlim([x1 x2]);
    xlabel('time (s)');
    title('pot and dep gain')
    box off;
subplot(5,1,4)
    plot(Tlap1, Params.k_pot.*q_sigmo(normD.*normP, Params.a_pot,Params.b_pot), 'r'); hold on
    plot(Tlap1, Params.k_dep.*q_sigmo(normD.*normP, Params.a_dep,Params.b_dep), 'b'); hold on
    xlim([x1 x2]);
    xlabel('time (s)');
    title('pot and dep rate: k*gain')
    box off;
subplot(5,1,5)
    plot(Tlap1, 10^12.*(Wmax-W(InputNum,NewLapIdx(LapNum))).*Params.k_pot.*q_sigmo(normD.*normP, Params.a_pot,Params.b_pot), 'r'); hold on
    plot(Tlap1, 10^12.*W(InputNum,NewLapIdx(LapNum)).*Params.k_dep.*q_sigmo(normD.*normP, Params.a_dep,Params.b_dep), 'b'); hold on
    xlim([x1 x2]);
    legend('(Wmax-W)*PotRate', 'W*DepRate')
    xlabel('time (s)');
    ylabel('pA')
    title('Potentiation and Depression')
    box off;

figure % BTSP variables for initially low input
InputNum = round(N/2+0.7*(N/2)); %dWmaxIn; %Nbittner
maxD = max(D(NewLapIdx(LapNum):NewLapIdx(LapNum+1)-1));
maxP = max(P(InputNum, NewLapIdx(LapNum):NewLapIdx(LapNum+1)-1));
normD = D(NewLapIdx(LapNum):NewLapIdx(LapNum+1)-1)./maxD;
normP = P(InputNum, NewLapIdx(LapNum):NewLapIdx(LapNum+1)-1)./maxP;
...
% % figure
% %     plot(0:dt:185/25, D(i-lapL:i)./max(D(i-lapL:i)), 'r'); hold on %plot(Tlap1, D(1:NewLapIdx(2)-1), 'r'); hold on
% %     plot(0:dt:185/25, CS(i-lapL:i), 'c'); hold on
% %     xlabel('time (s)'); 
% %     title('output raster + Post-before-Pre weight update')
% %     box off;
...
figure
[row_lap2,column_bin2] = find(CSbin2);
...
ax2 = nexttile
    ...
    scatter(column_bin2, row_lap2, 10, [0 1 1], 'filled'); hold on
...
%     legend('new lap', 'location', 'bestoutside')
    ...
    c = colorbar; c.Label.String = 'Synaptic weight (pA)'; c.Ticks = [0 max(W.*10^12,[], 'all')]; c.TickLabels = [0 max(W.*10^12,[], 'all')];
    title('W2')
    box off; axis square
nexttile
...
% x1 = addCStime-0.7; % in s
% x2 = addCStime+0.7; % in s 
% 
% subplot(4,1,1)
...
figure
x = 0:0.01:1;
plot(x,q_sigmo(x,Params.a_pot,Params.b_pot),'r'); hold on
plot(x,q_sigmo(x,Params.a_dep,Params.b_dep),'b'); hold on
legend('pot', 'dep')
title('gain')
box off
axis square
