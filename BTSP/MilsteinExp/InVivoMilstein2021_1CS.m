...
Params.Pamp = 80e-12; % peak weight change, in Amps (3*baseline EPSP for pairings with 5*10 EPSPs at 10Hz, with initial EPSP at 2mV, in Bittner et al. 2017)
...
if addCS == 1
    if ~isempty(CStime) %add 1 CS 3s before initial COM of current PF (which is in the middle)
    ...
    else % put CS at a time point corresponding to input 46 COM (closest to 77pA). Note: PFcom(46) = 84.25 cm for 185cm track, 
    [minval46, COMidx46] = min(abs(Lap1-PFcom(46)));
    COM46idx_Laps = find(Run==Run(COMidx46)); % initial COM idx on all laps (assuming constant speed)
    CS(COM46idx_Laps(CSlap)) = 1;
    end
end
...
if addCS == 1
    addCStime = CStimes{CSlap}; % time of CS during induction lap, where 0 is start of CSlap
    addCSloc = CSloc{CSlap}; % CS location on track, in cm (not binned)
    addCSbin = find(CSbin(CSlap,:)); % spatial bin of the added CS
...
muVbin.pre = mean(Vbin(1:CSlap-1,:),1);
...
muVbin.post = mean(Vbin(CSlap+1:end,:),1);
...
muVtime.pre = mean(cat(1, Vlo_laps{1:CSlap-1}), 1);
muVtime.post = mean(cat(1,Vlo_laps{CSlap+1:end}), 1);
