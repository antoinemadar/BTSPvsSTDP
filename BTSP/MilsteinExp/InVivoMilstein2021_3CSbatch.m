...
L = 300; % length of track in cm (300 for Can, 185 for Bittner and Milstein).
...
%     plot(Tlap1, Wraw{n,r}(InputNum,NewLapIdx(LapNum):NewLapIdx(LapNum+1)-1)./max(W2{n,r}(:)), 'b--'); hold on
    ...
    if Params.WUdyn == 1
    plot(Tlap1, Wtarget{n,r}(InputNum,NewLapIdx(LapNum):NewLapIdx(LapNum+1)-1)./max(W2{n,r}(:)), 'k'); hold on
    end
    xlim([x1 x2]);
    xlabel('time (s)'); 
...
% exportgraphics(t2,'BTSP_MilsteinExp1CSBatch_SheffieldParams_20pA_B.pdf','BackgroundColor','none', 'ContentType','vector')

% figure % CS-centered spatial deltaVm, method2
% for n = 1:2:N
%     for r = 1:repeats
% plot(CScenteredLocR{n,r}, DeltaVm_space{n,r}.*10^3, 'k-'); hold on
%     end
...
% figure % CS-centered DeltaVm_time profiles + mean
% CScenteredT = -Tlap1(end):dt:Tlap1(end);
% plot(CScenteredT,AllDeltaVm_timePad.*10^3,'Color', [0.5, 0.5, 0.5], 'LineWidth', 1); hold on
% plot(CScenteredT,MeanDeltaVm_timePad.*10^3, 'k', 'LineWidth', 3); hold on
% xline(0, 'c')
% xlim([-Tlap1(end) Tlap1(end)])
% xlabel('time from CS (s)'); ylabel('membrane potential (mV)')
% box off; axis square
...
figure % 3D plot: Distance from CS vs pre-induction Vm vs deltaVm
    subplot(1,2,1)
        scatter3([CScenteredLocR{:}], [AllmuVbin.pre_smooth].*10^3, [DeltaVm_space{:}].*10^3,  '.k')
        xlabel('Distance from CS (cm)'); 
        ylabel('pre-induction Vm (mV)')
        zlabel('deltaVm (mV)')
        axis square;
        view(-150, 14)
    subplot(1,2,2)
        colormap(brewermap(256,'*RdBu'));
        SurfX = [CScenteredLocR{:}]; SurfY = [AllmuVbin.pre_smooth].*10^3; SurfZ = [DeltaVm_space{:}].*10^3;
        [surface, Sgoodness, Soutput] = fit([SurfX' SurfY'], SurfZ','linearinterp');
        f = plot(surface); f.EdgeColor = 'none';
%         plot(surface, [SurfX' SurfY'], SurfZ');
        xlabel('Distance from CS (cm)'); 
        ylabel('pre-induction Vm (mV)')
        zlabel('deltaVm (mV)')
        axis square;
...
figure % same as above but flat map, like in Milstein2021: Distance from CS vs pre-induction Vm vs deltaVm
    colormap(brewermap(256,'*RdBu'));
    f = plot(surface); f.EdgeColor = 'none'; hold on
    xline(0,'k--', 'LineWidth', 2)
    xlabel('Distance from CS (cm)'); 
    ylabel('pre-induction Vm (mV)')
    zlabel('deltaVm (mV)')
...
% figure % 3D plot: Time from CS vs pre-induction Vm vs deltaVm
% AllmuVtime = [muVtime{:}];
% SurfX2 = [CScenteredTimeR{:}]; SurfY2 = ([AllmuVtime.pre]-Params.Vrest).*10^3; SurfZ2 = [DeltaVm_time{:}].*10^3;
% [surface2, Sgoodness2, Soutput2] = fit([SurfX2' SurfY2'], SurfZ2','linearinterp'); % note that 'lowess' method would give a smooth surface + go outside defined bounds, but takes too much time to run. Need to do that on downsampled data.
% f2 = plot(surface2); f2.EdgeColor = 'none';
% %         plot(surface, [SurfX' SurfY'], SurfZ');
% xlabel('Time from CS (s)'); 
...
%     axis square; box off; grid off
%     view(0, 90) % flat matrix view as in Milstein 2021
%     caxis([-max(f2.ZData(:)), max(f2.ZData(:))]);
%     c = colorbar; c.Label.String = '\Delta Vm (mV)';
% 
% figure % same as above but flat map, like in Milstein2021: Distance from CS vs pre-induction Vm vs deltaVm
%     colormap(brewermap(256,'*RdBu'));
...
figure % 3D plot: Time from CS vs pre-induction Vm vs deltaVm (Downsampled)
AllmuVtime = [muVtimeD{:}];
    subplot(1,2,1)
scatter3([CScenteredTimeD{:}], ([AllmuVtime.pre]-Params.Vrest).*10^3, [DeltaVm_timeD{:}].*10^3,  '.k')
xlabel('Time from CS (cm)'); 
...
figure % same as above but flat map, like in Milstein2021: Time from CS vs pre-induction Vm vs deltaVm
    colormap(brewermap(256,'*RdBu'));
    f2 = plot(surface2); f2.EdgeColor = 'none'; hold on
    xline(0,'k--', 'LineWidth', 2)
    xlabel('Time from CS (s)'); 
...
figure % same as above but flat map, like in Milstein2021: Distance from CS vs pre-induction Vm vs deltaVm
    ...
    disp([ 'max interpolated \DeltaVm =' num2str(max(f2.ZData(:))) ' mV']);
% 
% figure % 3D plot: dW_prepost = f(time or distance from CS, initial W), i.e. equivalent to deltaVm 3D plots 
% subplot(1,2,1)
% %      scatter3([CScenteredLocR{:}], cat(1,Wpre{:}).*10^12, cat(1,dW_prepost{:}).*10^12,  '.k')
%     scatter3([CScenteredTimeD{:}], cat(1,Wpre{:}).*10^12, cat(1,dW_prepost{:}).*10^12,  '.k')
%     xlabel('Time from CS (cm)'); 
%     ylabel('pre-induction Weight (pA)')
%     zlabel('\DeltaW (post-pre) (pA)')
%     axis square;
%     xlim([-8 8]); ylim([0 90]); zlim([-100 100]);
%     view(155, 15)
% subplot(1,2,2)
%     SurfY3 = cat(1,Wpre{:}).*10^12; SurfZ3 = cat(1,dW_prepost{:}).*10^12;
%     [surface3, Sgoodness3, Soutput3] = fit([SurfX2' SurfY3], SurfZ3,'linearinterp'); % note that 'lowess' method would give a smooth surface + go outside defined bounds, but takes too much time to run. Need to do that on downsampled data.
%     f3 = plot(surface3); f3.EdgeColor = 'none';
%     xlabel('Time from CS (cm)'); 
%     ylabel('pre-induction Weight (pA)')
%     zlabel('\DeltaW (post-pre) (pA)')
%     axis square;
...
%     colormap(brewermap(256,'*RdBu'));
% 
% figure % same as above but flat map
%     colormap(brewermap(256,'*RdBu'));
%     f3 = plot(surface3); f3.EdgeColor = 'none'; hold on
%     xline(0,'k--', 'LineWidth', 2)
%     xlabel('Time from CS (cm)'); 
%     ylabel('pre-induction Weight (pA)')
%     axis square; box off; grid off
%     view(0, 90) % flat matrix view as in Milstein 2021
%     caxis([-max(f3.ZData(:)), max(f3.ZData(:))]); xlim([-4.9, 4.9]); 
%     ylim([0 85]);
%     c = colorbar; c.Label.String = '\DeltaW (post-pre) (pA)';

figure % 3D plot: dWeff = f(time from CS, weight before change)-> need to downsample to be able to plot. Not enough memory otherwise.
clear X Y Z
X = [CScenteredTimeRW{:}]; Y = cat(1, W2bef{:}).*10^12; Z = cat(1,dWeffnz{:}).*10^12;
subplot(1,3,1)
scatter3(X(:), Y(:), Z(:),  '.k')
xlabel('time from CS (s)'); 
ylabel('Weight before plasticity (pA)')
zlabel('instant \DeltaW (pA)')
% zlim([-10 10])
view(150, 14)
axis square;
subplot(1,3,2)
...
figure % 3D plot: dWeff = f(time from CS, pre-induction weight) -> need to downsample to be able to plot. Not enough memory otherwise.
clear Y
subplot(1,3,1)
Y = cat(1,W2init_nz{:}).*10^12;
scatter3(X(:), Y(:), Z(:),  '.k')
xlabel('time from CS (s)'); 
ylabel('pre-induction Weight (pA)')
zlabel('instant \DeltaW (pA)')
...
figure % instant effective dW  = f(pre-induction weight) at time of CS
X = [CScenteredTimeRW{:}]; Y = cat(1, W2init_nz{:}).*10^12; Z = cat(1,dWeffnz{:}).*10^12;
Ycst = Y(X==0); Zcst = Z(X==0);
scatter(Ycst(:), Zcst(:),  '.k')
xlabel('pre-induction Weight (pA)')
ylabel('instant \DeltaW (pA)')
axis square; box off

figure % instant dW per spike = f(time from CS, pre-induction weight)
clear Z
Z = cat(1,dWmaxSpk{:}).*10^12;
subplot(1,3,1)
scatter3(X(:), Y(:), Z(:),  '.k')
xlabel('time from CS (s)'); 
ylabel('pre-induction Weight (pA)')
zlabel('\DeltaW per spike (pA)')
view(150, 14)
% view(90, 0)
axis square;
subplot(1,3,2)
...
figure % instant dW per spike = f(pre-induction weight) at time of CS
AlldWmaxSpk_CSt = cat(1,dWmaxSpk_CSt{:}).*10^12;
AllWinit_CSt = cat(1, W2init_CSt{:}).*10^12;
    subplot(1,2,1)
scatter(AllWinit_CSt, AlldWmaxSpk_CSt,  '.k')
xlabel('initial weight (pA)'); 
ylabel('dW per spike estimate (pA)')
title('evaluated at time of CS')
axis square; box off;
    subplot(1,2,2)
scatter(AllWinit_CSt, AlldWmaxSpk_CSt,  '.k')
ylim([0 max(AlldWmaxSpk_CSt)])
xlabel('initial weight (pA)'); 
ylabel('dW per spike estimate (pA)')
title(['evaluated at time of CS, dWmax ~ ' num2str(max(AlldWmaxSpk_CSt))])
axis square; box off;

figure % dWmax per spike = f(time from CS)
scatter(nonzeros(fdWmax_time(:)), nonzeros(effPamp(:)).*10^12,  'ok', 'filled', 'MarkerfaceAlpha', 0.4)
ylim([0 max([effPamp(:)].*10^12)+1])
xlabel('time from CS (s)'); 
ylabel('effective dWmax per spike estimate from max(dWeff) (pA)')
title('evaluated for  max weight change')
axis square;

figure % dWmax per spike = f(initial weight)
scatter(nonzeros(W2bef_F(:)).*10^12, nonzeros(effPamp(:)).*10^12,  'ok', 'filled', 'MarkerfaceAlpha', 0.4)
xlabel('initial weight (pA)')
ylabel('effective BTSP dWmax estimate (pA)')
ylim([0 max(effPamp(:).*10^12)+1])
title(['dWmax evaluated from max weight change ~ ' num2str(max(effPamp(:).*10^12)) 'pA'] ) 
axis square;

% max dVm
disp(['max \DeltaVm = ' num2str(max(SurfZ2)) ' mV'] );
...
cd('G:\My Drive\LabReasearch\Projects\Sheffield\Can1-Shifting\BTSP\BTSPHomeo_Optim2Milstein2021')
T = readtable('EffPamp_MilsteinParams.xlsx');
dWmax = [T.dWmax1, T.dWmax2, T.dWmax3];

figure
scatter(T.TestedPamp, dWmax, 'ok', 'filled', 'MarkerFaceAlpha', 0.5)
xlim([0 50])
ylim([0 max(dWmax,[], 'all')+1])
xlabel('Params.Pamp')
ylabel('effective dWmax estimate (pA)')
title('185cm track, animal speed: 25 cm/s')

T2 = readtable('EffPamp_SheffieldParams.xlsx');
dWmax2 = [T2.dWmax1, T2.dWmax2, T2.dWmax3];

figure
scatter(T2.TestedPamp, dWmax2, 'ok', 'filled', 'MarkerFaceAlpha', 0.5)
xlim([0 50])
ylim([0 max(dWmax2,[], 'all')+1])
xlabel('Params.Pamp')
ylabel('effective dWmax estimate (pA)')
