...
%% histograms and scatterplots of R2 and slopes
...
subplot(1,2,1)% proportion of forward, backward and stable/non-signif PFs (mean across animals)
    ...
    ylim([0 1]);
    ylabel('proportion of PFs');
    set(gca, 'XTick', [1, 2], 'XTickLabel', ['CA1N';'CA1F']);
    box off; %axis square;
subplot(1,2,2)
    histogram(DeltaPropCA1_NvsF, 'EdgeColor', mapCA1(2,:), 'LineWidth', 1, 'DisplayStyle','stairs', 'Normalization', 'probability');hold on
...
% figure delta of CA1FvsN and CA3F vs N for median slope on all PFs pooled
% (can't do it in DABEST website, because they dont do median)

figure
subplot(3,3,1) 
    plot([1,2], [meanPropCA1N meanPropCA1F], 'k-'); hold on
    plot([1,2], [meanPropCA3N meanPropCA3F], 'k-'); hold on
    errorbar(2 , meanPropCA1F, meanPropCA1F-propshiftCI_CA1F(1), meanPropCA1F-propshiftCI_CA1F(2), 'o', 'Color', mapCA1(1,:), 'MarkerFaceColor', mapCA1(1,:)); hold on
    errorbar(1 , meanPropCA1N, meanPropCA1N-propshiftCI_CA1N(1), meanPropCA1N-propshiftCI_CA1N(2), 'o', 'Color', mapCA1(2,:), 'MarkerFaceColor', mapCA1(2,:)); hold on
    errorbar(2 , meanPropCA3F, meanPropCA3F-propshiftCI_CA3F(1), meanPropCA3F-propshiftCI_CA3F(2), 'o', 'Color', mapCA3(1,:), 'MarkerFaceColor', mapCA3(1,:)); hold on
    errorbar(1 , meanPropCA3N, meanPropCA3N-propshiftCI_CA3N(1), meanPropCA3N-propshiftCI_CA3N(2), 'o', 'Color', mapCA3(2,:), 'MarkerFaceColor', mapCA3(2,:)); hold on
    ylabel('ratio of shifting PFs')
    xlim([0 3]); ylim([0 1])
    set(gca, 'XTick', [1, 2], 'XTickLabel', ['N';'F']);
    box off; axis square;
    title('bootstrapped 95% CIs')
subplot(3,3,2) % plot 2 histograms and CIs of DeltaPropCA1 and CA3 
    histogram(DeltaPropCA1_NvsF, 'EdgeColor', mapCA1(2,:), 'LineWidth', 1, 'DisplayStyle','stairs', 'Normalization', 'probability');hold on
    histogram(DeltaPropCA3_NvsF, 'EdgeColor', mapCA3(2,:), 'LineWidth', 1, 'DisplayStyle','stairs', 'Normalization', 'probability');hold on
    xline(mean(DeltaPropCA1_NvsF),'Color', mapCA1(2,:), 'LineWidth', 1.5 );hold on
    xline(mean(DeltaPropCA3_NvsF),'Color', mapCA3(2,:), 'LineWidth', 1.5 );hold on
    xline(0,'k-.', 'LineWidth', 1.5 );hold on
    xline(DeltaPropCA1_NvsF_CI, '--', 'Color', mapCA1(2,:), 'LineWidth', 1.5 );hold on
    xline(DeltaPropCA3_NvsF_CI, '--', 'Color', mapCA3(2,:), 'LineWidth', 1.5 );hold on
    box off; axis square;
    xlabel('\Delta ratio (N - F)'); ylabel('proba');
    title('bootstrapped distrib of diff ratio')
subplot(3,3,3) % plot DeltaDelta histogram and CI
    histogram(DeltaDelta, 'EdgeColor', 'c', 'LineWidth', 1, 'DisplayStyle','stairs', 'Normalization', 'probability');hold on
    xline(mean(DeltaDelta),'Color', 'c', 'LineWidth', 1.5 );hold on
    xline(0,'k-.', 'LineWidth', 1.5 );hold on
    xline(DeltaDelta_CI, '--', 'Color', 'c', 'LineWidth', 1.5 );hold on
    box off; axis square;
    xlabel('\DeltaDelta ratio (CA1NvsF - CA3NvsF)'); ylabel('proba');
    title('Interaction between 2 factors')

subplot(3,3,4) 
    plot([1,2], [meanPropBCA1N meanPropBCA1F], 'k-'); hold on
    plot([1,2], [meanPropBCA3N meanPropBCA3F], 'k-'); hold on
    errorbar(2 , meanPropBCA1F, meanPropBCA1F-propBackCI_CA1F(1), meanPropBCA1F-propBackCI_CA1F(2), 'o', 'Color', mapCA1(1,:), 'MarkerFaceColor', mapCA1(1,:)); hold on
    errorbar(1 , meanPropBCA1N, meanPropBCA1N-propBackCI_CA1N(1), meanPropBCA1N-propBackCI_CA1N(2), 'o', 'Color', mapCA1(2,:), 'MarkerFaceColor', mapCA1(2,:)); hold on
    errorbar(2 , meanPropBCA3F, meanPropBCA3F-propBackCI_CA3F(1), meanPropBCA3F-propBackCI_CA3F(2), 'o', 'Color', mapCA3(1,:), 'MarkerFaceColor', mapCA3(1,:)); hold on
    errorbar(1 , meanPropBCA3N, meanPropBCA3N-propBackCI_CA3N(1), meanPropBCA3N-propBackCI_CA3N(2), 'o', 'Color', mapCA3(2,:), 'MarkerFaceColor', mapCA3(2,:)); hold on
    ylabel('ratio of backward PFs')
    xlim([0 3]); ylim([0 1])
    set(gca, 'XTick', [1, 2], 'XTickLabel', ['N';'F']);
    box off; axis square;
    title('bootstrapped 95% CIs')
subplot(3,3,5) % plot 2 histograms and CIs of DeltaPropCA1 and CA3 
    histogram(DeltaPropBCA1_NvsF, 'EdgeColor', mapCA1(2,:), 'LineWidth', 1, 'DisplayStyle','stairs', 'Normalization', 'probability');hold on
    histogram(DeltaPropBCA3_NvsF, 'EdgeColor', mapCA3(2,:), 'LineWidth', 1, 'DisplayStyle','stairs', 'Normalization', 'probability');hold on
    xline(mean(DeltaPropBCA1_NvsF),'Color', mapCA1(2,:), 'LineWidth', 1.5 );hold on
    xline(mean(DeltaPropBCA3_NvsF),'Color', mapCA3(2,:), 'LineWidth', 1.5 );hold on
    xline(0,'k-.', 'LineWidth', 1.5 );hold on
    xline(DeltaPropBCA1_NvsF_CI, '--', 'Color', mapCA1(2,:), 'LineWidth', 1.5 );hold on
    xline(DeltaPropBCA3_NvsF_CI, '--', 'Color', mapCA3(2,:), 'LineWidth', 1.5 );hold on
    box off; axis square;
    xlabel('\Delta Bck ratio (N - F)'); ylabel('proba');
    title('bootstrapped distrib of diff ratio')
subplot(3,3,6) % plot DeltaDelta histogram and CI
    histogram(DeltaDeltaB, 'EdgeColor', 'c', 'LineWidth', 1, 'DisplayStyle','stairs', 'Normalization', 'probability');hold on
    xline(mean(DeltaDeltaB),'Color', 'c', 'LineWidth', 1.5 );hold on
    xline(0,'k-.', 'LineWidth', 1.5 );hold on
    xline(DeltaDeltaB_CI, '--', 'Color', 'c', 'LineWidth', 1.5 );hold on
    box off; axis square;
    xlabel('\DeltaDelta Back ratio (CA1NvsF - CA3NvsF)'); ylabel('proba');
    title('Interaction between 2 factors')

subplot(3,3,7) 
    plot([1,2], [meanPropFCA1N meanPropFCA1F], 'k-'); hold on
    plot([1,2], [meanPropFCA3N meanPropFCA3F], 'k-'); hold on
    errorbar(2 , meanPropFCA1F, meanPropFCA1F-propFwdCI_CA1F(1), meanPropFCA1F-propFwdCI_CA1F(2), 'o', 'Color', mapCA1(1,:), 'MarkerFaceColor', mapCA1(1,:)); hold on
    errorbar(1 , meanPropFCA1N, meanPropFCA1N-propFwdCI_CA1N(1), meanPropFCA1N-propFwdCI_CA1N(2), 'o', 'Color', mapCA1(2,:), 'MarkerFaceColor', mapCA1(2,:)); hold on
    errorbar(2 , meanPropFCA3F, meanPropFCA3F-propFwdCI_CA3F(1), meanPropFCA3F-propFwdCI_CA3F(2), 'o', 'Color', mapCA3(1,:), 'MarkerFaceColor', mapCA3(1,:)); hold on
    errorbar(1 , meanPropFCA3N, meanPropFCA3N-propFwdCI_CA3N(1), meanPropFCA3N-propFwdCI_CA3N(2), 'o', 'Color', mapCA3(2,:), 'MarkerFaceColor', mapCA3(2,:)); hold on
    ylabel('ratio of shifting PFs')
    xlim([0 3]); ylim([0 1])
    set(gca, 'XTick', [1, 2], 'XTickLabel', ['N';'F']);
    box off; axis square;
    title('bootstrapped 95% CIs')
subplot(3,3,8) % plot 2 histograms and CIs of DeltaPropCA1 and CA3 
    histogram(DeltaPropFCA1_NvsF, 'EdgeColor', mapCA1(2,:), 'LineWidth', 1, 'DisplayStyle','stairs', 'Normalization', 'probability');hold on
    histogram(DeltaPropFCA3_NvsF, 'EdgeColor', mapCA3(2,:), 'LineWidth', 1, 'DisplayStyle','stairs', 'Normalization', 'probability');hold on
    xline(mean(DeltaPropFCA1_NvsF),'Color', mapCA1(2,:), 'LineWidth', 1.5 );hold on
    xline(mean(DeltaPropFCA3_NvsF),'Color', mapCA3(2,:), 'LineWidth', 1.5 );hold on
    xline(0,'k-.', 'LineWidth', 1.5 );hold on
    xline(DeltaPropFCA1_NvsF_CI, '--', 'Color', mapCA1(2,:), 'LineWidth', 1.5 );hold on
    xline(DeltaPropFCA3_NvsF_CI, '--', 'Color', mapCA3(2,:), 'LineWidth', 1.5 );hold on
    box off; axis square;
    xlabel('\Delta ratio (N - F)'); ylabel('proba');
    title('bootstrapped distrib of diff ratio')
subplot(3,3,9) % plot DeltaDelta histogram and CI
    histogram(DeltaDeltaF, 'EdgeColor', 'c', 'LineWidth', 1, 'DisplayStyle','stairs', 'Normalization', 'probability');hold on
    xline(mean(DeltaDeltaF),'Color', 'c', 'LineWidth', 1.5 );hold on
    xline(0,'k-.', 'LineWidth', 1.5 );hold on
    xline(DeltaDeltaF_CI, '--', 'Color', 'c', 'LineWidth', 1.5 );hold on
    box off; axis square;
    xlabel('\DeltaDelta ratio (CA1NvsF - CA3NvsF)'); ylabel('proba');
    title('Interaction between 2 factors')

%% multinomial logistic regression 
% for testing effect of CAfield and VRfamiliarity on proportions of B, F and N shifting PFs
Tmnr = Tok(:,["animalID","Region","f_n","shiftDir"]);
mlr1 = fitmnr(Tok, 'shiftDir ~ Region + f_n');
mlr2 = fitmnr(Tmnr,'shiftDir ~ Region + f_n + animalID', CategoricalPredictors=["Region", "f_n", "animalID"]);
